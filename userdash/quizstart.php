...
<h1 class="h3 mb-2 text-gray-800" id="agentTitle" data-localize="User_quiz">User Quiz</h1>
...
//  if ($_SESSION["tenant"] == 'lsv_mastertenant' || @$_GET['id'] == $_SESSION["agent"]['agent_id']) {
   ?>
    ...
    <div class="text-center">
      ...
      $curdate = date('Y-m-d'); 
      $fetchqry = "SELECT * FROM `lsv_visitorquiz_report` WHERE `user_id`='$user_id' AND `score` IS NULL AND is_eligible = 1 AND timestamp LIKE '%$curdate%'  ORDER BY id DESC";
      ...
      $result = mysqli_query($con, $fetchqry);
        ...
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
          ...
          if(is_null($row['score'])){
            ...
            $pending_quiz_count++;
           echo '<button class="button" id="mybut" onclick="myFunction()">Start <?= $_GET["category"]?> Quiz</button>';   
            }else{ 
              echo @$row['score'];
        ...
        ?>        
                ...
                <button class="button" id="mybut" onclick="myFunction()">Start <?= $_GET['category']?> Quiz</button>
        <?php 
            }
          
        }
        
        if($pending_quiz_count < 1){echo "There is no quiz currently available for you .";}
    ...
    <div id="myDIV" style="padding: 10px 30px;">
      ...
      <form action="quizresult.php" method="post" id="form">
        ...
        <input type="hidden" name="roomId" value="<?php echo $_GET['room'];?>">
        
          <?php
            $fetchqry = "SELECT * FROM lsv_quiz WHERE category = '$_GET[category]' AND status=1 ";
            $result = mysqli_query($con, $fetchqry);
            $num = mysqli_num_rows($result);
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
          ?>
